    ...
    def do_insert_scn(self, cursor, item):
    	...
    	try:
    		...
    		insert into food_safety_scn(major, title, status, publish_time, update_time, source, content_len, content, url, pic_num, pic_url) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
    				"""
    		cursor.execute(insert_sql, (item['major'], pymysql.escape_string(item['title']), "新增", item['publish'], ndt, item['source'], item['content_len'], pymysql.escape_string(item['content']), item['url'],item['pic_num'], item['pic_url']));
    	except pymysql.Error as e:
    		#print ("except", e);
    		if (e.args[0] == 1062):   #已有数据，则更新
    			...
    			where title='%s'" %(item['source'], item['content_len'], item['major'], item['publish'], ndt, item['url'], item['pic_num'],item['pic_url'], pymysql.escape_string(item['title']));
    			#print(update_sql);
    			cursor.execute(update_sql);
    	return 0;
